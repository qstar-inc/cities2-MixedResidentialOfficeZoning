...
namespace MixedResidentialOfficeZoning
{
    public class Mod : IMod
    {
        ...
        public static void ProcessThumbnail()
        {
            ...
            var thumbNotification = _notificationUISystem.AddOrUpdateNotification("starq-mixed-office-thumbnail-process",
        ...
        protected static bool CheckExistingFiles(string ailCustomFolder, NotificationUISystem.NotificationInfo thumbNotification = null)
        {
            ...
            if (thumbNotification != null && count == 330)
            {
                ...
                thumbNotification.progressState = ProgressState.Complete;
                thumbNotification.progress = (4 / 4 * 100);
                _notificationUISystem.RemoveNotification("starq-mixed-office-thumbnail-process");
            }
            return count == 330;
        ...
        protected static void FindAndCopyFiles(string modFolder, string ailCustomFolder, NotificationUISystem.NotificationInfo thumbNotification)
        {
            ...
            foreach (string directory in directories)
            {
                ...
                {
                    Match match = Regex.Match(Path.GetFileName(file), @"(([EUNA]{2})_Mixed([LeftRight\d]{2,7})_L([\d])_([\dx]{3})\.png)");


                    if (match.Success)
                    {
                        string newFileName = match.Groups[2].Value + "_MixedOffice" + match.Groups[3].Value + "_L" + match.Groups[4].Value + "_" + match.Groups[5].Value + ".png";
